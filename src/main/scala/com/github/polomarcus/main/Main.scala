...
object Main {
  def main(args: Array[String]) {
    ...
    // Tips: use a groupBy
    val climateVsOtherNewsCount = enrichedDataset.groupBy("containsWordGlobalWarming").count()
    climateVsOtherNewsCount.show()
    ...
    val OthersNewsCount = spark.sql("SELECT COUNT(*) FROM news WHERE containsWordGlobalWarming = false").collect()(0)(0)
    println(s"Climate-related news count: $climateNewsCount")
    println(s"Non-climate-related news count: $OthersNewsCount")
